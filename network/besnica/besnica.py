...
from network.besnica.besnica_reduced_network import create_besnica_reduced_asymmetric_network
from network.besnica.besnica_symmetrical_network import create_besnica_symmetrical_network


def create_besnica_network():
    # empty net
    ...
    for loads_bus in buses_of_loads:
        ...
        if loads_bus == "5111":
            pp.create_asymmetric_load(net, bus=b, p_a_mw=avg / 1000, p_b_mw=avg / 1000, p_c_mw=avg / 1000, q_a_mvar=0,
                                      q_b_mvar=0, q_c_mvar=0, sn_mva=loads_consumption / 1000, name=f"{loads_bus}b",
                                      scaling=1.0, index=None, in_service=True, type='wye')

    # create last load
    pp.create_asymmetric_load(net, bus=dict_of_buses['1'], p_a_mw=0, p_b_mw=0.011, p_c_mw=0, q_a_mvar=0, q_b_mvar=0,
                              q_c_mvar=0, sn_mva=0.011, name='1', scaling=1.0, index=None, in_service=True, type='wye')
    ...
    for line in all_network_as_graph:
        ...
        dict_of_lines[line] = pp.create_line(net, from_bus=dict_of_buses[str(act[0])],
                                             to_bus=dict_of_buses[str(act[1])],
                                             length_km=act[2] / 1000, std_type=act[3], name=line)

    # adds the zero sequence parameters of the standard types to the net model
    pp.add_zero_impedance_parameters(net)

    # run three-phase power flow
    pp.runpp_3ph(net, calculate_voltage_angles=True, init='auto', max_iteration='auto', tolerance_mva=1e-08,
...
def create_network(network_type):
    if configuration.config.getboolean("input", "load_network_from_file") and exists(besnica_network_file):
        net = pp.from_excel(besnica_network_file)
        return net

    if configuration.config.getboolean("scenario", "asymmetric"):
        if network_type == "besnica_reduced":
            net = create_besnica_reduced_asymmetric_network()
            name = "reduced_asymmetric"
        else:
            net = create_besnica_network()
            name = "asymmetric"
    else:
        if network_type == "besnica_reduced":
            net = create_besnica_reduced_network()
            name = "reduced_symmetric"
        else:
            net = create_besnica_symmetrical_network()
            name = "symmetric"

    if configuration.config.getboolean("output", "save_network_as_excel"):
        besnica_filename = configuration.config.get("input", "besnica_network_file", fallback=name)
        pp.to_excel(net, join(path, besnica_filename))
    return net
