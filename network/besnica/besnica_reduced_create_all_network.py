...
def sort_load_buses_as_consumptions(buses_of_loads):
    """Simple bubble-sort"""
...
def create(base_network, mid_buses):
    ...
    for line in mid_dict:
        ...
        for piece in mid_dict[line]:
            ...
            if i == 1:  # the first part of this line
                ...
                before = base_dict[line][1] * 100 + i
            i += 1
    # now, the original lines that contain loads are splitted into correct, "chain/part-by-part" lines, so the old
    # ones has to be deleted
    keys_to_del = []
    for line in base_dict:
    ...
    buses_of_loads = sort_load_buses_as_consumptions(buses_of_loads)
